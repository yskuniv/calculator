  ...
  describe '#simplify' do
    ...
    context 'when 1 is given as a base' do
      it 'returns the identity of Rational' do
        expect(Exponential[1, Rat[2, 3]].simplify).
          to eq Rat[1, 1]
      end
    end
    ...
    context 'when an integer exponent is given' do
      it 'returns a simple Rational' do
        expect(Exponential[2, Rat[3, 1]].simplify).
          to eq Rat[8, 1]
      end
    end
    ...
    context 'when a simple rational exponent is given' do
      ...
      context 'in case that only a Exponential is expected to return' do
        it 'returns a proper Exponential' do
          expect(Exponential[2, Rat[2, 3]].simplify).
            to eq Exponential[4, Rat[1, 3]]
        end
      end

      context 'in case that a CFactor is expected to return' do
        it 'returns a proper CFactor' do
          expect(Exponential[24, Rat[1, 2]].simplify).
            to eq CFactor[Rat[2, 1], Exponential[6, Rat[1, 2]]]
        end
      end
    ...
    context 'when a complicated exponent is given' do
      ...
      context 'when e.n < e.d' do
        it 'returns a proper CFactor' do
          expect(Exponential[144, Rat[2, 3]].simplify).
            to eq CFactor[Rat[12, 1], Exponential[12, Rat[1, 3]]]
        end
      end

      context 'when e.n > e.d' do
        it 'returns a proper CFactor' do
          expect(Exponential[144, Rat[8, 3]].simplify).
            to eq CFactor[Rat[248832, 1], Exponential[12, Rat[1, 3]]]
        end
      end
  ...
  describe '#multiply' do
    ...
    context 'in ordinary case' do
      it 'returns a correct value' do
        expect(Exponential[2, Rat[1, 3]].multiply(Exponential[2, Rat[1, 3]])).
          to eq Exponential[4, Rat[1, 3]]
      end
    end
    ...
    context 'when an incompatible one is given' do
      it 'raises an Error' do
        expect { Exponential[2, Rat[1, 3]].multiply(Exponential[3, Rat[1, 3]]) }.
